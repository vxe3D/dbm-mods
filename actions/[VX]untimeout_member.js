module.exports = {
  name: "[VX]untimeout_member",
  displayName: "Untimeout Member",
  section: "# VX - Member(s)",
  meta: {
    version: "3.2.0",
    actionVersion: "3.0.0",
  ...
  variableStorage(data, varType) {
    ...
    return [data.resultVar, "Untimeout"];
  },
  ...
  subtitle(data, presets) {
    ...
    const parts = [member, reason, variable].filter((v) => v && String(v).trim() !== "");
    return parts.join(" | ");
  },

  fields: ["member", "varName", "reason", "resultVar", "resultVarType"],

  html(isEvent, data) {
    const actionVersion = (this.meta && typeof this.meta.actionVersion !== "undefined") ? `${this.meta.actionVersion}` : "???";
    const actionFilename = (this.name ? this.name + ".js" : "[VX]untimeout_member.js");
  ...
  preInit() {
    const f = window.__VX_ACTION_FILENAME||"[VX]untimeout_member.js", l = window.__VX_ACTION_VERSION||"0.0.0", c = (a,b) => {a=a.split('.').map(Number),b=b.split('.').map(Number);for(let i=0;i<Math.max(a.length,b.length);i++){let n1=a[i]||0,n2=b[i]||0;if(n1!==n2)return n1-n2;}return 0;}, githubUrl = `https://github.com/vxe3D/dbm-mods/blob/main/actions%2F${encodeURIComponent(f)}`;
  ...
  async action(cache) {
    ...
    if (Array.isArray(member)) {
      this.callListFunc(member, "disableCommunicationUntil", [null, reason])
        .then(() => { storeResult(true); this.callNextAction(cache); })
        .catch((err) => { storeResult(false); this.displayError(data, cache, err); });
