module.exports = {
  ...
  window.__VX_ACTION_FILENAME = actionFilename;
    ...
    <div style="padding-top: 8px;">
      ...
      <textarea id="replacements" rows="5" placeholder="𝐄𝐱𝐚𝐦𝐩𝐥𝐞:\nundefined=>Nie znaleziono\nerror=>Wystąpił błąd" style="width: 99%; font-family: white-space: nowrap; resize: none;"></textarea>
    </div>
  ...
  replacements.forEach(([oldValue, newValue]) => {
    ...
    if (/^<a?:\w+:\d+>$/.test(newValue)) {
      // Jeśli zamieniamy emotkę (np. "0" -> "<a:no:123>"), to upewniamy się, że jest traktowana jako osobny element
      const regex = new RegExp(`(?<!<a?[:\\w]*)\\b${oldValue}\\b(?![:\\w]*>)`, 'g');
      result = result.replace(regex, newValue);
    } else {
      // Normalna zamiana tekstu, również dla "undefined"
