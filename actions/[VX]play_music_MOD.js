module.exports = {
  ...
  const { Readable } = require("stream");
    ...
    let eqFilter = buildEqFilter(eqBands);
  ...
  let requestedById = null;
    ...
    if (!serverQueue) {
      ...
      const playSong = async (songUrl) => {
        ...
        if (eqFilter) {
          // Zamień fluent-ffmpeg na natywne wywołanie ffmpeg
          const ffmpegArgs = [
            "-i", "pipe:0",
            "-f", "webm",
            "-acodec", "libopus",
          ...
          const ffmpegProc = spawn("ffmpeg", ffmpegArgs, { stdio: ["pipe", "pipe", "ignore"] });
          // Przekaż strumień yt-dlp do ffmpeg stdin
