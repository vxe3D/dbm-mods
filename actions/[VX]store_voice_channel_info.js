module.exports = {
  name: "[VX]store_voice_channel_info",
  displayName: "Store Voice Channel Info",
  ...
  variableStorage(data, varType) {
    ...
    switch (info) {
      case 0: dataType = "VoiceChannel"; break;
      case 1: dataType = "String"; break;
      case 2: dataType = "String"; break;
      case 3: dataType = "Number"; break;
      case 4: dataType = "CategoryChannel|ForumChannel|TextChannel|VoiceChannel|ThreadChannel|null"; break;
      case 5: dataType = "String"; break;
      case 6: dataType = "Boolean"; break;
      case 7: dataType = "Boolean"; break;
      case 8: dataType = "String"; break;
      case 9: dataType = "Number"; break;
      case 10: dataType = "Map"; break;
      case 11: dataType = "Number"; break;
      case 12: dataType = "Number"; break;
      case 13: dataType = "String"; break;
      case 14: dataType = "Boolean"; break;
      case 15: dataType = "Boolean"; break;
      case 16: dataType = "Boolean"; break;
      case 17: dataType = "Boolean"; break;
  ...
  html(isEvent, data) {
    ...
    const actionFilename = (this.name ? this.name + ".js" : "[VX]store_voice_channel_info.js");
    ...
    return `
      ...
      <channel-input dropdownLabel="Source Voice Channel" selectId="channel" variableContainerId="varNameContainer" variableInputId="varName"></channel-input>
      <br><br><br>
      <div style="padding-top: 8px;">
        <span class="dbminputlabel">Source Info</span><br>
        <select id="info" class="round">
          <optgroup label="⭐ General">
            <option value="0">Voice Object</option>
            <option value="1">Voice ID</option>
            <option value="2">Voice Name</option>
            <option value="3">Voice Position</option>
            <option value="4">Voice Parent</option>
            <option value="5">Voice Parent ID</option>
          </optgroup>
          <optgroup label="👥 Member(s)">
          <option value="8">Members (Mentions)</option>
          <option value="9">Member Count</option>
          <option value="10">Speaking (Map)</option>
          </optgroup>
          <optgroup label="⚙️ Properties">
            <option value="6">Voice NSFW</option>
            <option value="7">Voice Viewable</option>
            <option value="11">Bitrate</option>
            <option value="12">User Limit</option>
            <option value="13">RTC Region</option>
          </optgroup>
          <optgroup label="🧩 Flags">
            <option value="14">Full?</option>
            <option value="15">Joinable? [BOT]</option>
            <option value="16">Speakable? [BOT]</option>
            <option value="17">Deletable?</option>
            <option value="18">Manageable?</option>
          </optgroup>
        </select>
      </div>
      <br>
  ...
  preInit() {
    const f = window.__VX_ACTION_FILENAME||"[VX]store_voice_channel_info.js", l = window.__VX_ACTION_VERSION||"0.0.0", c = (a,b) => {a=a.split('.').map(Number),b=b.split('.').map(Number);for(let i=0;i<Math.max(a.length,b.length);i++){let n1=a[i]||0,n2=b[i]||0;if(n1!==n2)return n1-n2;}return 0;}, githubUrl = `https://github.com/vxe3D/dbm-mods/blob/main/actions%2F${encodeURIComponent(f)}`;
  ...
  async action(cache) {
    ...
    const voice = await this.getChannelFromData(data.channel, data.varName, cache);
    if (!voice) return this.callNextAction(cache);

    const info = parseInt(data.info, 10);
    let result;
    switch (info) {
      case 0: result = voice; break;
      case 1: result = voice.id; break;
      case 2: result = voice.name ?? null; break;
      case 3: result = voice.position ?? null; break;
      case 4: result = voice.parent ?? null; break;
      case 5: result = voice.parentId ?? null; break;
      case 6: result = voice.nsfw ?? null; break;
      case 7: result = voice.viewable ?? null; break;
      case 8: result = voice.members ? voice.members.map(m => `<@${m.id}>`).join(', ') : null; break;
      case 9: result = voice.members ? voice.members.size : null; break;
      case 10: result = voice.speaking ?? null; break;
      case 11: result = voice.bitrate ?? null; break;
      case 12: result = voice.userLimit ?? null; break;
      case 13: result = voice.rtcRegion ?? null; break;
      case 14: result = voice.full ?? null; break;
      case 15: result = voice.joinable ?? null; break;
      case 16: result = voice.speakable ?? null; break;
      case 17: result = voice.deletable ?? null; break;
      case 18: result = voice.manageable ?? null; break;
    }
