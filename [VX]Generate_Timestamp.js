module.exports = {
  name: "Generate Discord Timestamp",
  section: "# VX - Time",
  subtitle(data) {
...
`;
  ...
  async action(cache) {
    ...
    // Pobieramy styl w zależności od tego, czy manualInput jest podany
    ...
    if (manualInput) {
      ...
      const normalizedInput = dateInput.includes(" ") ? dateInput.replace(/ /g, "T") : dateInput;
    ...
    // Zwracamy tylko jeden poprawny timestamp
    this.storeValue(
      discordTimestamp,
      parseInt(data.storage),
      this.evalMessage(data.varName, cache),
